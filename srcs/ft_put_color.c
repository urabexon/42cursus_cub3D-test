...
/*   Updated: 2025/03/03 20:03:12 by hurabe           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_put_floor_and_ceiling(t_data *data)
{
	...
	floor_color = data->floor_rgb[0] << 16 | \
		data->floor_rgb[1] << 8 | data->floor_rgb[2];
	ceiling_color = data->ceiling_rgb[0] << 16 | \
		data->ceiling_rgb[1] << 8 | data->ceiling_rgb[2];
	y = 0;
	while (y < HEIGHT / 2)
	{
		x = 0;
